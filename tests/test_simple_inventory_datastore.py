...
class TestInventoryLoad:
    ...
    def test_broker_inventory(self):
        ...
        assert "endpoint" in self.inventory.broker, "Broker inventory has no 'endpoint' data"
        
    def test_workers_inventory(self):    
        assert self.inventory.workers.data, "No workers data"
        assert isinstance(self.inventory.workers.data, dict), "Workers data not a dictionary"    
        assert isinstance(self.inventory.workers.path, str), "Workers inventory path not a string"    
...
class TestWorkersInventory:
    ...
    def test_get_item(self):
        ...
        assert isinstance(nornir_worker_1, dict), "No dictionary data for nornir-worker-1"
    ...
    def test_dict_merge(self):
        ...
        assert "foobar" in nornir_worker_2["groups"], "'foobar' group data was not merged"
        
    def test_value_overwirte(self):
        nornir_worker_2 = self.inventory.workers["nornir-worker-2"]
        
        assert nornir_worker_2["groups"]["valueoverwrite"]["port"] == 777, "'valueoverwrite.port' not overriden by nested group"
